...
namespace SmartPark.Tests.Services
{
    public class EspaciosServiceTests
    {
        ...
        private static async Task SeedBaseEntities(DbContext context)
        {
            context.Set<EstadosEspacio>().Add(new EstadosEspacio
            {
                Id = 1,
                Nombre = "Disponible"
            });

            context.Set<TiposVehiculo>().Add(new TiposVehiculo
            {
                Id = 1,
                Nombre = "Carro"
            });
